...
There's a [chess demo](http://localhost:3001/chess), too. The board is the input, legal moves are Choice options, and a Score question rates the position. Choose You, Kev, Jev, or GPT-6-Luna for either side. For Jev, put `AI_GATEWAY_API_KEY` in `playground/.env.local`; for Luna, add `OPENAI_API_KEY`. Restart the playground server after adding keys. Keys stay on the server, and neither hosted model needs the local Kev API.

Games and decision traces are saved as JSON files in `runs/chess-games/`. Existing browser games are imported on first load. Open **Decision trace** to inspect the latest request and response, or **Download game traces** to export the game. Every new model move records the shared Choice/Score request, response, token usage, latency, selected action, and whether you sampled it. Luna uses the Responses API with structured output and medium reasoning effort. Its probabilities are generated estimates normalized for sampling, not token probabilities or calibrated confidence. Traces label them `generated_estimates` and preserve the original numbers in `generated_probabilities`.

To collect Kev self-play games without a browser, start `kev.serve`, then run `cd playground && npm run chess:traces -- --games 10 --concurrency 4 --out ../runs/chess-traces.jsonl`. The runner plays up to four games at once; moves within each game remain sequential. `--concurrency` defaults to 1. Each JSONL line is a game with the request, returned move distribution, sampled action, board position, and final result at every ply. Games stopped by `--maxPlies` have a null result. The API rounds probabilities to two decimals, so these traces support analysis and supervised data preparation, but policy-gradient training needs unrounded action probabilities or logits from the model.

For Luna self-play, run `cd playground && npm run chess:traces -- --model gpt-6-luna --games 1 --maxPlies 20 --out ../runs/luna-chess.jsonl`. The runner loads `playground/.env.local` and calls OpenAI directly, using the same adapter as the browser. Each ply makes one paid API call. Both runners save the full response alongside the sampled action in the same JSONL format.

For Jev against Luna, run `cd playground && npm run chess:traces -- --white jev --black gpt-6-luna --games 100 --concurrency 16 --out ../runs/jev-luna-chess.jsonl`. Set `AI_GATEWAY_API_KEY` and `OPENAI_API_KEY` in `playground/.env.local`. The runner calls both hosted models directly; it needs no local Kev server. Each active game makes one request at a time, and completed games append to the JSONL file.
